...
"""
This module is a wrapper for PySide and PyQt:
It was forked from https://github.com/pyqtgraph

* Automatically import either PyQt or PySide depending on availability
* Allow to import QtCore/QtGui MadQt.Qt without specifying which Qt wrapper
  you want to use.
* Declare QtCore.Signal, .Slot in PyQt
...
FRAMEWORK = os.getenv('MAD_FRAMEWORK')
...
if FRAMEWORK is not None:
    try:
        ...
        raise ModuleNotFoundError(f"Environment variable MAD_FRAMEWORK is set to '{os.getenv('MAD_FRAMEWORK')}', but no module with this name was found.")
...
if FRAMEWORK is None:
    raise Exception("MadQt requires one of PyQt5, PyQt6, PySide2 or PySide6; none of these packages could be imported.")
...
def _loadUiType(uiFile):
    ...
    if pyside2uic is None:
        uic_executable = FRAMEWORK.lower() + '-uic'
...
# For historical reasons, MadQt maintains a Qt4-ish interface back when
...
if FRAMEWORK == PYQT5:
    ...
    raise ValueError("Invalid Qt lib '%s'" % FRAMEWORK)
...
if FRAMEWORK in [PYQT5, PYQT6, PYSIDE2, PYSIDE6]:
    ...
    def lazyGetattr(name):
        ...
        if name not in whitelist:
            warnings.warn(
                "Accessing QtWidgets through QtGui is deprecated and will be removed sometime"
...
if FRAMEWORK in [PYSIDE2, PYSIDE6]:
...
USE_PYSIDE = FRAMEWORK == PYSIDE
USE_PYQT4 = FRAMEWORK == PYQT4
USE_PYQT5 = FRAMEWORK == PYQT5
...
if m is not None and list(map(int, m.groups())) < versionReq:
    ...
    raise Exception('MadQt requires Qt version >= %d.%d  (your version is %s)' % (versionReq[0], versionReq[1], QtVersion))
...
def mkApp(name=None):
    ...
    **Arguments:**
    ...
    if QAPP is None:
        ...
        QAPP = QtWidgets.QApplication(sys.argv or ["MadQt"])
